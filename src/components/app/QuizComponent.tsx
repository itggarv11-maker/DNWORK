...
const QuizComponent: React.FC<QuizComponentProps> = ({ questions, sourceText, subject }) => {
    ...
    useEffect(() => {
        return () => ttsService.cancel(); // Stop any speech when component unmounts
    }, []);
    ...
    const handleSpeak = (textToSpeak: string) => {
        // Remove LaTeX for clearer speech
        const cleanText = textToSpeak.replace(/\$[^$]+\$/g, 'the formula').replace(/\\sqrt/g, 'square root of');
                        ...
                        return (
                            <Card variant="light" key={index} className={`!shadow-md !p-4 border-2 !bg-white/60 ${borderColor}`}>
                                <div className="flex justify-between items-start">
                                    <MathRenderer text={`${index + 1}. ${q.question}`} className="font-semibold text-slate-800" />
                                ...
                                {q.type === 'mcq' && (
                                    <div className="mt-2 text-sm space-y-1">
                                        <p className={`p-2 rounded-md ${isCorrectMCQ ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                            Your answer: <MathRenderer text={answeredQ.userAnswer || ''} className="font-bold inline-block" />
                                            {isCorrectMCQ ? 
                                                <CheckCircleIcon className="inline w-4 h-4 ml-2 text-green-600" /> :
                                                <XCircleIcon className="inline w-4 h-4 ml-2 text-red-600" />
                                            }
                                        </p>
                                        {!isCorrectMCQ && <p className="p-2 rounded-md bg-green-100 text-green-800">Correct answer: <MathRenderer text={q.correctAnswer || ''} className="font-bold inline-block" /></p>}
                                    </div>
                                )}
                                 {q.type === 'written' && (
    ...
    return (
        <div className="space-y-4">
            ...
            <Card variant="light" className="!shadow-none border border-slate-300 !bg-white/60">
                <div className="flex justify-between items-start">
                    <MathRenderer text={`${currentQuestionIndex + 1}. ${currentQuestion.question}`} className="font-semibold text-lg mb-4 text-slate-800 flex-grow" />
                    <button onClick={() => handleSpeak(currentQuestion.question)} className="p-2 text-slate-500 hover:bg-slate-200 rounded-full flex-shrink-0 ml-2"><SpeakerWaveIcon className="w-5 h-5"/></button>
                </div>
                    ...
                    {!showResult ? (
                        ...
                        <div className="space-y-4">
                            ...
                            <Card variant="light" className="bg-indigo-100/70 border-indigo-200/80 border">
                                ...
                                <MathRenderer text={currentQuestion.explanation} className="mt-2 text-sm text-indigo-700" />
                            </Card>
